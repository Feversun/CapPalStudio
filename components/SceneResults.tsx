      ...
      <div className="max-w-6xl mx-auto space-y-8">
        ...
        <div className="flex justify-center">
            <div className={`relative bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden group transition-all duration-500 ${getAspectClass()} ${!currentImage && 'w-full h-96'}`}>
                ...
                {currentImage ? (
                    <>
                        <img src={currentImage} alt="Scene Result" className="w-full h-full object-cover" />
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-4">
                    {history.map((item) => (
                        <div 
                            ...
                            className={`aspect-square rounded-xl overflow-hidden cursor-pointer border-2 transition-all group relative ${currentImage === item.imageUrl ? 'border-indigo-600 ring-2 ring-indigo-100' : 'border-transparent hover:border-indigo-300'}`}
                        >
                            <img src={item.imageUrl} alt="History" className="w-full h-full object-cover" loading="lazy" />
