...
const StickerGrid: React.FC<StickerGridProps> = ({ stickers, progress, onRegenerate, finalPrompt }) => {
  ...
  const handleDownloadOrShare = async (e: React.MouseEvent, sticker: Sticker) => {
    ...
    try {
        // Attempt to convert base64/url to Blob
        const response = await fetch(sticker.imageUrl);
        const blob = await response.blob();
        const fileName = `sticker-${sticker.emotion.toLowerCase().replace(/\s+/g, '-')}.png`;
        const file = new File([blob], fileName, { type: 'image/png' });

        // Try Native Share API (Mobile friendly - "Save Image" option in iOS)
        ...
        // Fallback: Desktop classic download
        ...
        console.warn("Share failed, falling back to direct link", error);
        // Absolute fallback
        const link = document.createElement('a');
        link.href = sticker.imageUrl;
